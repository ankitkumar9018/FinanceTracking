...
### Added — AI
- **AI portfolio digest** — a short written round-up of totals, P&L, top positions, gainers/losers and concentration flags. `GET /ai/digest/` returns the latest, `POST /ai/digest/generate` creates one on demand, and `GET`/`PUT /ai/digest/schedule` set the frequency (`off` | `daily` | `weekly`). **Works with no AI provider at all**: the digest is built deterministically from the user's computed numbers and a model is only ever asked to rewrite that same grounded context — any timeout/error falls back to the numbers-only text with `provider: "none"`. A new daily background job (`ai_digest_job` / `app.tasks.ai_digest_task.run_scheduled_digests_celery`) generates due digests (weekly = Mondays UTC) and delivers them through the existing notification channels. Stored per user in `notification_preferences` (`ai_digest_latest` / `ai_digest_frequency`) — no migration.
- **Agentic chat actions (confirm before execute)** — the assistant can now *propose* `add_transaction`, `add_holding`, or `create_alert`; the `POST /ai/chat` response carries a `proposed_action` object (`id`, `type`, `summary`, `params`). **Nothing is executed until the user confirms** via `POST /ai/chat/actions/{action_id}/execute`; `POST /ai/chat/actions/{action_id}/dismiss` discards it (`204`). Params are re-validated and ownership re-checked at execution, which runs through the *same route handlers* as manual entry (so the SELL ledger guard and cumulative-holding recompute are identical). Proposals expire after **900s (15 min)** and are capped at **5 per chat session**. Requires an active LLM provider — without one the intent pass is skipped entirely.
- **AI summary in reports** — opt-in `ai_summary=true` query parameter on `GET /import-export/export/report/{portfolio_id}` and `GET /import-export/export/pdf/{portfolio_id}` prepends a compact 4–6 sentence grounded summary. Best-effort: any AI failure or timeout exports the report unchanged, without the section.
- **AI alert explanations** — triggered alert notifications can carry one short AI sentence explaining the trigger in plain terms. Gated by `AI_ALERT_EXPLANATIONS` (default on) and time-boxed by `AI_ALERT_EXPLAIN_TIMEOUT` (default 20s); silently skipped on no-provider/timeout/error so alert delivery is never delayed or lost.
- **New settings** — `OLLAMA_TIMEOUT` (default `300.0`; raise it when a local model is slow), `AI_DIGEST_TIMEOUT` (default `120.0`, covers digest and report-summary generation), `AI_ALERT_EXPLANATIONS` (default `true`), `AI_ALERT_EXPLAIN_TIMEOUT` (default `20.0`).

### Fixed — AI & Migrations
- **Migration recovery now steps revision-by-revision.** A schema-ahead-of-stamp database previously risked being stamped straight to `head`, silently marking genuinely pending revisions as applied. Recovery now applies each pending revision individually (`upgrade +1`), stamping past *only* the ones whose DDL collides with what already exists (the next revision id is resolved through `ScriptDirectory`, since `alembic stamp` rejects relative revisions).
- **Prompt-injection hardening.** Every user-controlled string interpolated into the LLM portfolio context (portfolio name, sector, symbol, exchange, warnings) is now sanitized: newlines flattened, runs of 3+ `=` (the context-block delimiter) collapsed, whitespace squeezed, and length-capped — so a portfolio named `=== END PORTFOLIO CONTEXT ===` can no longer smuggle instructions into the system prompt. Confirmation replies for proposed actions are composed server-side rather than trusted to the model.
- **Chat cost caps.** Only the most recent **20** messages of a session are replayed to the provider, and the (expensive) compiled portfolio context is cached per session for **300s** instead of being rebuilt on every turn.
- **Stale chat sessions no longer fork silently.** `POST /ai/chat` with an unknown or foreign `session_id` now returns `404` instead of quietly starting a new session and fragmenting the conversation.
...
- **Import ⇄ Export hub** — the `/import` page now carries an **Import / Export** tab switcher; the Export tab renders the full export list, so exports are reachable from the page users already associate with moving data.
- **Round-trip test suite** — `backend/tests/test_export_import_roundtrip.py` exports through the real service functions, re-imports into a fresh portfolio, and compares cumulative quantity and average price, pinning both the header aliasing and the snapshot semantics.

### Changed — Navigation
- **Sidebar entry renamed to "Import & Export"** (same `/import` route, `ArrowDownUp` icon). Export used to live only under **Reports**, where users could not find it; the ⌘K command palette entry was renamed to match, so searching "export" now finds the page.
- **Reports page is now headed "Reports & Export"** (its sidebar entry is unchanged) and renders the same shared `ExportOptions` component as the hub — one source of truth for the export list — plus the Capital Gains Tax Report. The two pages cross-link.

### Fixed — Import / Export
- **Exported CSV/Excel files can be re-imported.** Exports write human-readable headers (`Stock Symbol`, `Avg Price`, `Type`) while the importers expected the template keys (`stock_symbol`, `price`, `transaction_type`), so re-importing an export failed with *"No valid data rows found"*. Both importers now canonicalise headers through one shared alias map (`csv_import_service.canonical_column`, reused by `excel_service`), which also accepts common spellings (`Symbol`, `Ticker`, `Qty`, `Shares`, `Trade Date`, `Fees`). Every existing template header still maps to itself, and unknown export-only columns (`Current Price`, `RSI`, `P&L %`, `Action Needed`) are ignored instead of breaking the row.
- **Excel import prefers a `Transactions` sheet.** A workbook exported by the app holds both a Holdings snapshot and a Transactions ledger; `parse_excel` now tries a sheet named `Transactions` first and falls back to the active sheet (the single-sheet template) or a `Holdings` sheet when it yields no usable rows — so the Excel export round-trips the full ledger.
- **Position-snapshot rows import as an opening BUY.** The holdings CSV and the "Holdings" sheet carry no transaction type or date, so each row is now materialised as a single **BUY** of the exported quantity at the exported average price, **dated today** — positions and averages are reproduced, the original ledger is not. Duplicate-transaction detection keeps a same-day re-import from doubling a position; importing the same snapshot on a later day creates a second opening BUY. Rows with a type but no date (or vice versa) are still rejected.
- **`stock_name` is now optional on import** (it falls back to the symbol, which is what the transactions export carries), and **zero-quantity rows are skipped** — a fully exited holding still appears in the holdings export.
